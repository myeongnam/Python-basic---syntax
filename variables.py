...
# 아스키코드라는 문자와 매칭되는 테이블을 사용하여 약속된 숫자값으로 저장
...
# multi line으로 문자열을 표현하고 싶으면 """쌍따옴표 3개나 '''홀따옴표 3개를 사용하면 된다.
# a = """hello
...
# # 그리고 C 라는 변수에 두 문자열을 더한 값을 넣어 c를 출력
...
# # python에서 인덱스란, 기본적으로 특정위치를 가르키는 용도로 사용
...
# #6번째 문자부터 끝까지 변수 b에 담아 출력
# a = "python is fun"
# print (a[6:])
# # a[x:y:z] 여기에서 z는 z-1 개씩 건너뛰고,
# # 2번쨰 이상 7번째 미만 문자열 중에 1개씩 건너뛰고 b에 담아 출력
# print(a[2:7:2])
...
# 포맷팅을 왜 쓰는가? 
# 1) 따옴표를 여러번 안 닫아도된다
# 2)문자열을 직접 삽입하면 1회성으로 coding할수 밖에 없지만 
# 포맷팅은 변수값을 삽입할 수 있다.
...
# weight = input("몸무게가 몇킬로그램이신가요?")
# # b = "my age is %d and weight is %f kg" % (int(age)), (float(weight))
# # print (b)
...
# # find : 대상 문자열에서 지정한 문자가 몇번쨰 index에 있는지 찾는 함수
# # index : find와 같은 기능
# print(a.find('o'))
# print(a.index('o'))

# # 없는 문자를 찾을때는 -1 return
# print(a.find('x'))
